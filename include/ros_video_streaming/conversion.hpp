...
inline types::PixFormatList convert_pix_formats(const std::vector<v4l2_fmtdesc>& pix_formats) {
  auto pix_format_list = types::PixFormatList{};
  pix_format_list.reserve(pix_formats.size());
  const auto converter = [](const v4l2_fmtdesc& fmt) { return *formats::fourcc2format(fmt.pixelformat); };
